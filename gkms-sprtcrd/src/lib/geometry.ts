...
/**
 * サムネイル最下端のレアリティ帯（R=水色 / SR=金 / SSR=虹）。
 * 実測（フィクスチャ較正）: SR は金一色（hue≈44・低ばらつき）、
 * SSR は虹（hue のばらつきが大）、R は低彩度の水色（hue≈197・sat≈0.15）。
 */
export const RARITY_BAND = { y0: 0.93, y1: 0.99, x0: 0.14, x1: 0.86 } as const
