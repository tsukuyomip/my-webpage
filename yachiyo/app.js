...
(() => {
  ...
  function step(dt, t) {
    ...
    // 螺旋のあいだは上限を上げておく。裾を回るのに要る速さに余裕が無いと、
    // 外周の魚が上限に張りついて半径を保てない。
    const vmax = (240 + 520 * S.boost) * speedK * (1 - SH.calm * 0.8) * (SH.hr > 1 ? 2.1 : 1);
      ...
      if (shOn) {
        ...
        if (SH.hr > 1 && jw > 0) {
          ...
          const tgy = SH.hbase - hRel * SH.hlen + R * SH.hk * sn
                    + (sz_[i] - 1.05) * 30 * spread * Math.min(1, hRel / 0.15);
          ...
          if (lap_[i] !== lap) {
            lap_[i] = lap;
            // 湧くのは必ず水の中。上の裾の置き方でほぼ保証されるが、
            // 画面の形によっては届かないことがあるので、ここでも押さえる。
            x = tgx; y = Math.max(tgy, waterY + 10);
            vx = 0; vy = 0;
            px_[i] = x; py_[i] = y;
          }
          ...
          // 遠い魚ほど弱めるが 0 にはしない。0 にすると、いちど遅れた魚が
          // 公転をやめて円の内側を最短距離で突っ切り、そのまま戻れなくなる。
          const near = dd < 90 ? 1 : dd < 430 ? 0.34 + 0.66 * (430 - dd) / 340 : 0.34;
          if (near > 0) {
            const wv = SH.hw * near;
            dxT += -sn * R * wv;
            dyT += cs * R * SH.hk * wv;
  ...
  function stepShow(dt) {
    ...
    if (d.id === 'suikomi') {
      ...
      if (hu > 0) {
        ...
        // 裾は水面より十分下に置く。魚は裾で湧く（一周した魚を位置ごと移す）ので、
        // 湧いてから見えきるまでの帯が丸ごと水の中に入っていないと、
        // 何もない空中に魚がふっと現れて見える。
        // 以前は裾を演目の進みで 0.94H → 0.84H と持ち上げていて、終盤には
        // 湧きの帯の上端が水面より 80px も上に出ていた。持ち上げるのをやめ、
        // さらに楕円の縦ぶれ（±R·hk）ぶん余裕を見る。
        const baseMin = H * 0.95;
        const need = waterY + 0.07 * (baseMin - SH.hapex) + SH.hr * 1.29 * SH.hk;
        SH.hbase = clamp(need, baseMin, H * 0.995);
        SH.hapex = H * (0.86 - 0.84 * grow);       // 頂点（下から上へ伸びる）
        SH.hlen = Math.max(60, SH.hbase - SH.hapex);
        // 裾がわずかに画面をはみ出すくらい。ただし高さでも頭打ちにする。
        // 横向きだと W だけで決めると円錐が水深より広くなり、湧く帯が
        // どうやっても水面から出てしまう（縦より横が 2 倍以上長いため）。
        SH.hr = Math.min(W * 0.47, H * 0.45) * ramp;
        // 裾から頂点までの総ひねり。ここを時間で無限に増やすと（以前は
        // 高さごとに角速度を変えて位相を積んでいた）巻き数が際限なく増え、
        // 13 秒の終わりには 6 周を超えて模様が潰れる＝螺旋が壊れる。
        // 総ひねりは 3.5 周で頭打ちにして、形を保ったまま回す。
        const turns = TAU * (0.6 + 2.9 * grow);      // 裾から頂点までの総ひねり
        SH.hturns = turns * show.dir;
        ...
        // ■ 回る速さは「裾の魚が追いつける速さ」から逆算する
        // 螺旋に沿って登ると、登るだけで角度が turns × 登る速さ ぶん動く。
        // 3.5 周の螺旋を毎秒 0.075 周ぶん登ると、それだけで 1.65rad/s。
        // 全体の回り 0.89rad/s と足すと 2.5rad/s になり、裾（半径 237px）では
        // 目標が 600px/s で動く。魚の上限は 432px/s、目標へ寄る頭打ちは
        // 265px/s なので、外周の魚は原理的に追いつけない。遅れた魚は下の
        // near ゲートから外れて公転成分を失い、円の内側を突っ切る。
        // ＝終盤に中央へ収束してまっすぐな帯に見えていた正体。
        //
        // そこで「裾の接線速度」に予算を置き、全体の回りと登りで分け合う。
        const RIM = 320;                             // 裾に出させる接線速度
        const budget = RIM / Math.max(SH.hr * 1.29, 1);   // 使える角速度 rad/s
        SH.hw = Math.min(0.30, budget * 0.25) * show.dir; // 1/4 を全体の回りへ
        // 残りを登りにあてる。これで裾の魚も置いていかれない。
        SH.hflow += dt * Math.max(0.012, (budget - Math.abs(SH.hw)) / turns);
        SH.hph += SH.hw * dt;
        show.y = SH.hapex;
      ...
      // 31〜34s の流れ。画面ぜんぶが同じ方向へ、20〜30 秒かけて流れる。
      //
      ...
      // 山の数は長さから決めて、1 山を 6 秒前後に保つ。u に対して固定の山数に
      // すると、長い回では 1 山 15 秒になって緩急が読めなくなる。
      const humps = Math.max(2, Math.round(show.dur / 6.2));
