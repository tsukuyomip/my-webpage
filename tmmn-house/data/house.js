...
export const floors = [
  ...
  // ==========================================================================
  {
    ...
    openings: [
      ...
      { type: 'door',    from: [3.4, 2.4],  to: [3.4, 3.2],  label: '土間／主寝室' },
      { type: 'archway', from: [1.0, 0.5],  to: [1.0, 1.3],  label: '主寝室／WIC' },
    ...
    stairs: [
      { id: 'stair-main', from: [4.3, 6.9], to: [5.2, 3.5], steps: 15, dir: 'up', label: 'スケルトン階段' },
    ],
  },
  ...
  // ==========================================================================
  {
    ...
    openings: [
      ...
      { type: 'railing', from: [4.3, 4.1], to: [4.3, 5.8],  label: '階段室 手すり' },   // 4.1より北は降り口なので開ける
      ...
      { type: 'archway', from: [3.4, 2.2], to: [3.4, 3.2],  label: '廊下／キッチン' },
    ...
    stairs: [
      { id: 'stair-main-2f', from: [4.3, 3.5], to: [5.2, 6.9], steps: 15, dir: 'down', label: 'スケルトン階段（下り）' },
    ],
    /** 階段室の抜き（2Fの床と1Fの天井に開ける穴）。頭がぶつからない範囲で開ける */
    slabHoles: [
      [[4.3, 3.5], [5.2, 3.5], [5.2, 5.8], [4.3, 5.8]],   // 北端は階段の着地の板に合わせる
    ],
  },
];
...
export const designNotes = {
  ...
  resolved: [
    ...
    {
      issue: '【3D化で発覚】ベッド・棚・観葉植物が扉の前をふさぎ、開かない／通れない扉が14か所あった',
      fix: '家具そのものを当たり判定に取り込み、建具の前後に人が立てる空きがあるかを実測する検査を追加。'
        + '主寝室の扉の位置とベッドの向き、トイレの器具の寄せ方、洗面のカウンター長さ、浴槽の位置、'
        + 'ヌックのベンチの長さと奥行、キッチンの背面収納などを見直して、全14か所を解消した。'
        + '棚とクローゼットは、建具の前を自動で避けて作られるようにしてある。',
    },
    {
      issue: '【俯瞰モードで発覚】中庭のシンボルツリーの樹冠が階全体を覆い、2Fの壁を突き破っていた。'
        + 'ファミリークローゼットの服が玄関の吹抜へはみ出していた。階段の上り切りが手すりで塞がれ、床にも隙間があった',
      fix: '樹冠を中庭の内側の枠に収め、枝先と葉を折り返すようにした。家具はL字の室でも実際のポリゴンに沿って切り詰め、'
        + '「家具が室からはみ出していないか」を実測する検査を追加。階段は最上段を上階の床の縁まで届く着地の板にし、'
        + '階段室の手すりを降り口の手前で止めた。',
    },
    {
